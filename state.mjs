...
export function defaultSettings() {
  return {
    // Chrome CDP is the default because Google/SSO providers commonly block
    // embedded Electron auth surfaces.
...
export function normalizeSettings(input) {
  ...
  const out = {
    browserBackend: ['chrome-cdp', 'electron'].includes(String(s.browserBackend || '').trim().toLowerCase())
    ...
    chromeExecutablePath: typeof s.chromeExecutablePath === 'string' && s.chromeExecutablePath.trim() ? s.chromeExecutablePath.trim() : null,
    chromeProfileMode: ['agentify', 'persistent', 'isolated', 'existing', 'attach'].includes(String(s.chromeProfileMode || '').trim().toLowerCase())
      ? String(s.chromeProfileMode || '').trim().toLowerCase()
      : d.chromeProfileMode,
    chromeProfileName: typeof s.chromeProfileName === 'string' && s.chromeProfileName.trim() ? s.chromeProfileName.trim() : d.chromeProfileName,
