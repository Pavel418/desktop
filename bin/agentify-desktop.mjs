...
GUI options:
  --browser-backend chrome-cdp|electron
  --chrome-binary /path/to/chrome
  --chrome-debug-port 9222
  --chrome-profile-mode isolated|existing
...
function electronBin() {
  const local = path.join(packageRoot, 'node_modules', '.bin', process.platform === 'win32' ? 'electron.cmd' : 'electron');
